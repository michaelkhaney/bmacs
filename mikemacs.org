...
This is Michael K. Haney's personal Emacs configuration.

** Personal Information

#+BEGIN_SRC emacs-lisp
  (setq user-full-name "Michael Haney"
        user-mail-address "michael.k.haney@gmail.com"
...
#+BEGIN_SRC emacs-lisp
(defvar mikemacs-font (font-spec :family "Iosevka" :size 25))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(eval-and-compile
  (defvar mikemacs-leader-key "SPC"
    "The leader prefix key, for global commands.")

  (defvar mikemacs-localleader-key "SPC m"
...
#+BEGIN_SRC emacs-lisp
(eval-and-compile
  (defvar mikemacs-emacs-dir (expand-file-name user-emacs-directory)
    "The path to the emacs.d directory.")

  (defvar mikemacs-local-dir (concat mikemacs-emacs-dir ".local/")
    "Root directory for local Emacs files. Use this as permanent storage for files
  that are safe to share across systems (if this config is symlinked across
  several computers).")

  (defvar mikemacs-host-dir (concat mikemacs-local-dir "@" (system-name))
    "Directory for hostname-specific file storage. Used by `mikemacs-etc-dir' and
  `mikemacs-cache-dir'.")

  (defvar mikemacs-etc-dir (concat mikemacs-host-dir "/etc/")
    "Host-namespaced directory for non-volatile storage. These are not deleted or
  tampored with by MIKEMACS functions. Use this for dependencies like servers or
  config files that are stable (i.e. it should be unlikely that you need to delete
  them if something goes wrong).")

  (defvar mikemacs-cache-dir (concat mikemacs-host-dir "/cache/")
    "Host-namespaced directory for volatile storage. Deleted when `mikemacs/reset' is
  ...
  (defvar mikemacs-packages-dir (concat mikemacs-local-dir "packages/")
    "Where package.el and quelpa plugins (and their caches) are stored.")


  (dolist (dir (list mikemacs-local-dir mikemacs-etc-dir mikemacs-cache-dir (expand-file-name "elpa" mikemacs-packages-dir)))
...
#+BEGIN_SRC emacs-lisp
(defvar mikemacs-post-init-hook nil
  "A list of hooks run after MIKEMACS initialization is complete, and after
`mikemacs-init-hook'.")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defvar mikemacs-large-file-size 1
  "Size (in MB) above which the user will be prompted to open the file literally
to avoid performance issues. Opening literally means that no major or minor
modes are active and the buffer is read-only.")

(defvar mikemacs-large-file-modes-list
  '(archive-mode tar-mode jka-compr git-commit-mode image-mode
    doc-view-mode doc-view-mode-maybe ebrowse-tree-mode pdf-view-mode)
  "Major modes that `mikemacs/check-large-file' will ignore.")
#+END_SRC

*** Misc

#+BEGIN_SRC emacs-lisp
(defvar mikemacs-popups '()
...
(setq gnutls-verify-error t
      ...
      nsm-settings-file (expand-file-name "network-security.data" mikemacs-cache-dir))
#+END_SRC
...
(add-hook 'mikemacs-post-init-hook #'(lambda () (setq gc-cons-threshold 16777216
...
#+BEGIN_SRC emacs-lisp
  (eval-and-compile
    (setq package-user-dir (expand-file-name "elpa" mikemacs-packages-dir)))
...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (setq quelpa-checkout-melpa-p nil
        ...
        quelpa-dir (expand-file-name "quelpa" mikemacs-packages-dir)
        quelpa-update-melpa-p nil)
...
(eval-and-compile
  ...
  (defvar mikemacs-evil-state-alist
    '((?n . normal)
      (?v . visual)
      (?i . insert)
      (?e . emacs)
...
;; specials
  (defvar mikemacs--keymaps nil)
  (defvar mikemacs--prefix  nil)
  (defvar mikemacs--defer   nil)
  (defvar mikemacs--local   nil)

(defun mikemacs--keybind-register (key desc &optional modes)
...
(defun mikemacs--keyword-to-states (keyword)
  "Convert a KEYWORD into a list of evil state symbols.

For example, :nvi will map to (list 'normal 'visual 'insert). See
`mikemacs-evil-state-alist' to customize this."
  (cl-loop for l across (substring (symbol-name keyword) 1)
           if (cdr (assq l mikemacs-evil-state-alist))
             collect it
           else
...
States
    ...
    This can be customized with `mikemacs-evil-state-alist'.
...
Example
  ...
  (let ((mikemacs--keymaps mikemacs--keymaps)
        (mikemacs--prefix  mikemacs--prefix)
        (mikemacs--defer   mikemacs--defer)
        (mikemacs--local   mikemacs--local)
    ...
    (while rest
      ...
      (cond
       ...
       ((keywordp key)
        (cond ((eq key :leader)
               (push 'mikemacs-leader-key rest)
               (setq key :prefix
                     desc "<leader>"))
              ((eq key :localleader)
               (push 'mikemacs-localleader-key rest)
               (setq key :prefix
                     desc "<localleader>")))
        (pcase key
          ...
          (:map*    (setq mikemacs--defer t) (push :map rest))
          (:map
            (setq mikemacs--keymaps (mikemacs-enlist (pop rest))))
          (:mode
            (setq modes (mikemacs-enlist (pop rest)))
            (unless mikemacs--keymaps
              (setq mikemacs--keymaps
          ...
          (:prefix
            (let ((def (pop rest)))
              (setq mikemacs--prefix `(vconcat ,mikemacs--prefix (kbd ,def)))
              (when desc
                (push `(mikemacs--keybind-register ,(key-description (eval mikemacs--prefix))
                                                ,desc ',modes)
                      forms)
                (setq desc nil))))
          (:local
           (setq mikemacs--local t))
          (_ ; might be a state mikemacs--prefix
           (setq states (mikemacs--keyword-to-states key)))))
       ...
       ((or (stringp key)
        ...
        (unwind-protect
            (catch 'skip
              ...
              (when (stringp key)
                (setq key (kbd key)))
              (when mikemacs--prefix
                (setq key (append mikemacs--prefix (list key))))
              (unless (> (length rest) 0)
                (user-error "map! has no definition for %s key" key))
              (setq def (pop rest))
              (when desc
                (push `(mikemacs--keybind-register ,(key-description (eval key))
                                              ,desc ',modes)
                      forms))
              (cond ((and mikemacs--local mikemacs--keymaps)
                     (push `(lwarn 'mikemacs-map :warning
                                   "Can't local bind '%s' key to a keymap; skipped"
                                   ,key)
                           forms)
                     (throw 'skip 'local))
                    ((and mikemacs--keymaps states)
                     (dolist (keymap mikemacs--keymaps)
                       (push `(,(if mikemacs--defer 'evil-define-key 'evil-define-key*)
                               ',states ,keymap ,key ,def)
                             forms)))
                    (states
                     (dolist (state states)
                       (push `(define-key
                                ,(intern (format "evil-%s-state-%smap" state (if mikemacs--local "local-" "")))
                                ,key ,def)
                             forms)))
                    (mikemacs--keymaps
                     (dolist (keymap mikemacs--keymaps)
                    ...
                    (t
                     (push `(,(if mikemacs--local 'local-set-key 'global-set-key) ,key ,def)
                           forms))))
          (setq states '()
                mikemacs--local nil
                desc nil)))
...
#+BEGIN_SRC emacs-lisp
(eval-and-compile
  (defun mikemacs--resolve-hook-forms (hooks)
    (cl-loop with quoted-p = (eq (car-safe hooks) 'quote)
             for hook in (mikemacs-enlist (doom-unquote hooks))
  ...
  (defvar mikemacs--transient-counter 0)
  ...
  then it detaches itself."
    ...
    (let ((append (eq (car forms) :after))
          (fn (intern (format "mikemacs-transient-hook-%s" (cl-incf mikemacs--transient-counter)))))
      `(when ,hook
         (fset ',fn
               (lambda (&rest _)
                 ,@forms
...
`args'."
  ...
  (let ((hook-fn 'add-hook)
    ...
    (let ((hooks (mikemacs--resolve-hook-forms (pop args)))
          (funcs
           (let ((val (car args)))
...
(setq-default
 ...
 ;; files
 abbrev-file-name             (concat mikemacs-local-dir "abbrev.el")
 auto-save-list-file-name     (concat mikemacs-cache-dir "autosave"))
#+END_SRC

**** Pcache File Location

#+BEGIN_SRC emacs-lisp
(setq pcache-directory (concat mikemacs-cache-dir "pcache"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(setq savehist-file (concat mikemacs-cache-dir "savehist")
      history-length 500
      savehist-save-minibuffer-history t
      savehist-autosave-interval nil ; save on kill only
      savehist-additional-variables '(kill-ring search-ring regexp-search-ring)
      save-place-file (concat mikemacs-cache-dir "saveplace"))
...
#+BEGIN_SRC emacs-lisp
(defun mikemacs|enable-line-numbers (&optional arg)
...
(defun mikemacs|disable-line-numbers ()
  "Disable the display of line numbers."
  (if (boundp 'display-line-numbers)
      (setq display-line-numbers nil)
    (nlinum-mode -1)))

(add-hook 'prog-mode-hook #'mikemacs|enable-line-numbers)
(add-hook 'text-mode-hook #'mikemacs|enable-line-numbers)
(add-hook 'conf-mode-hook #'mikemacs|enable-line-numbers)
#+END_SRC
...
(setq-default
 ...
 ;; Bookmarks
 bookmark-default-file (concat mikemacs-etc-dir "bookmarks")
...
(defun mikemacs/check-large-file ()
  "Check if the buffer's file is large (see `mikemacs-large-file-size'). If so, ask
for confirmation to open it literally (read-only, disabled undo and in
fundamental-mode) for performance sake."
  (let* ((filename (buffer-file-name))
         (size (nth 7 (file-attributes filename))))
    (when (and (not (memq major-mode mikemacs-large-file-modes-list))
               size (> size (* 1024 1024 mikemacs-large-file-size))
               (y-or-n-p
...
(add-hook 'find-file-hook #'mikemacs/check-large-file)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defvar mikemacs|yank-indent-threshold 1000 "don't auto indent over 1000 lines")

(defvar mikemacs|indent-sensitive-modes '(conf-mode coffee-mode haml-mode python-mode slim-mode yaml-mode)
  "modes to limit auto indentation on")


(defmacro mikemacs|advise-commands (advice-name commands class &rest body)
...
(defun mikemacs|yank-advised-indent-function (beg end)
  "Do indentation, as long as the region isn't too large."
  (if (<= (- end beg) mikemacs|yank-indent-threshold)
      (indent-region beg end nil)))

(mikemacs|advise-commands
  "indent" (evil-paste-before evil-paste-after) around
  "If current mode is not one of mikemacs-indent-sensitive-modes
  indent yanked text (with universal arg don't indent)."
  ad-do-it
  (evil-with-single-undo
    (if (and (not (equal '(4) (ad-get-arg 0)))
             (not (member major-mode mikemacs-indent-sensitive-modes)) ;; TODO indent-sensitive-modes
             (derived-mode-p 'prog-mode))
        (let ((transient-mark-mode nil)
              (save-undo buffer-undo-list))
          (mikemacs|yank-advised-indent-function (region-beginning)
                                                (region-end))))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(set-face-attribute 'default nil :font mikemacs-font)
#+END_SRC
...
(use-package golden-ratio
             ...
             (setq golden-ratio-auto-scale 0
                   ...
                   golden-ratio-exlude-modes '(imenu-list-major-mode
                                               eshell-mode
                                               ansi-term-mode
                                               pdf-view-mode
                                               mu4e-main-mode
...
(add-to-list 'golden-ratio-extra-commands 'evil-window-bottom)
(add-to-list 'golden-ratio-extra-commands 'evil-window-top)
#+END_SRC
...
(use-package ivy
  :demand t
  :init
  (add-hook 'mikemacs-post-init-hook #'ivy-mode)
  :config
  (setq ivy-height 12
        ivy-wrap t
        ...
        ;; MIKEMACS allow input prompt value to be selectable
...
(use-package smex
  ...
  :config
  (setq smex-save-file (concat mikemacs-cache-dir "/smex-items"))
  (smex-initialize))
#+END_SRC
...
(use-package company-statistics
  :after company
  :config
  (setq company-statistics-file (concat mikemacs-cache-dir "company-stats-cache.el"))
...
(use-package persp-mode :demand t
  ...
  (setq wg-morph-on nil
        ...
        persp-save-dir (concat mikemacs-etc-dir "workspaces/"))
  ...
  (add-hook 'mikemacs-post-init-hook #'(lambda () (persp-mode 1))))
...
(use-package evil-escape
  ...
  (add-hook 'mikemacs-post-init-hook #'evil-escape-mode)
  :config
...
(use-package evil-snipe
  ...
  (add-hook 'mikemacs-post-init-hook #'evil-snipe-override-mode))
#+END_SRC
...
(use-package undo-tree
  ...
  (setq undo-tree-auto-save-history nil
        ...
        (list (cons "." (concat mikemacs-cache-dir "undo-tree-hist/")))))
#+END_SRC
...
Collection of interfaces to GitHub integrated into Magit workflows.
...
(use-package magithub
  ...
  :after magit
  :init
  (setq magithub-dir (concat mikemacs-cache-dir "magithub"))
...
| =H H=                | Open current repository in browser |
...
(use-package vi-tilde-fringe
  ...
  :init
  (add-hook 'mikemacs-post-init-hook #'global-vi-tilde-fringe-mode)
  :config
...
(use-package doom-themes
  ...
  (add-hook 'mikemacs-post-init-hook #'doom-themes-org-config)

  ;; blink mode-line on errors
  (add-hook 'mikemacs-post-init-hook #'doom-themes-visual-bell-config)

  ;; Add file icons to doom-neotree
  (add-hook 'mikemacs-post-init-hook #'doom-themes-neotree-config)
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-derived-mode +doom-dashboard-mode special-mode
    (format "MIKEMACS v%s" "0.1.0")
    "Major mode for the MIKEMACS dashboard buffer."
    (read-only-mode +1)
  ...
  if in a GUI/non-daemon session."
    ...
    (when (and (display-graphic-p) (not (daemonp)))
      (let ((default-directory mikemacs-emacs-dir))
    ...
    (let ((all-the-icons-scale-factor 1.45)
      ...
      (mapc (lambda (btn)
            ...
            `(("Homepage" "mark-github"
               (browse-url "https://github.com/michaelkhaney/mikemacs"))
              ...
              ("Edit emacs.d" "tools"
               (find-file (expand-file-name "init.el" mikemacs-emacs-dir)))
              ("Edit mikemacs" "tools"
               (find-file (expand-file-name "mikemacs.org" mikemacs-emacs-dir)))))))
...
(defun +doom-modeline--buffer-file-name-relative (&optional include-project)
  ...
  (let ((root (mikemacs-project-root))
        (active (active)))
    (if (null root)
        (propertize "%b" 'face (if active 'doom-modeline-buffer-file))
      (let* ((modified-faces (if (buffer-modified-p) 'doom-modeline-buffer-modified))
...
~/Projects/FOSS/emacs/lisp/comint.el => ~/P/F/emacs/lisp/comint.el"
  (let* ((project-root (mikemacs-project-root))
...
(add-hook 'mikemacs-post-init-hook #'+doom-modeline|init)
...
(use-package evil-goggles
  ...
  (add-hook 'mikemacs-post-init-hook #'evil-goggles-mode t))
       ...
       '(((lambda (limit)
            (let (case-fold-search)
              (and (re-search-forward hl-todo-regexp limit t)
                   (memq 'font-lock-comment-face (mikemacs-enlist (get-text-property (point) 'face))))))
...
#+BEGIN_SRC emacs-lisp
(defun mikemacs*blink-cursor-maybe (orig-fn &rest args)
    ...
    (unless (or (derived-mode-p 'term-mode)
                (equal point
                       (save-excursion (goto-char (window-start))
                                       (point-marker))))
      (mikemacs/blink-cursor))))

;;;###autoload
(defun mikemacs/blink-cursor (&rest _)
...
(use-package nav-flash
  ...
  (advice-add #'windmove-do-window-select :around #'mikemacs*blink-cursor-maybe)
  (advice-add #'recenter :around #'mikemacs*blink-cursor-maybe)

  (after! evil
    (advice-add #'evil--jumps-jump   :after #'mikemacs/blink-cursor)
    (advice-add #'evil-window-top    :after #'mikemacs/blink-cursor)
    (advice-add #'evil-window-middle :after #'mikemacs/blink-cursor)
    (advice-add #'evil-window-bottom :after #'mikemacs/blink-cursor)))
...
;   (add-hook 'mikemacs-post-init-hook #'solaire-mode-swap-bg t)
;
...
(use-package hl-line ; built-in
  ...
  (when (boundp 'display-line-numbers)
    ...
    (setq hl-line-range-function #'mikemacs--line-range))
  ...
  (defun mikemacs|disable-hl-line () (hl-line-mode -1))

  (add-hook 'evil-visual-state-entry-hook #'mikemacs|disable-hl-line)
...
(unless (boundp 'display-line-numbers)
  (use-package nlinum
    ...
    (defun mikemacs|init-nlinum-width ()
      "Calculate line number column width beforehand (optimization)."
      (setq nlinum--width
            (length (save-excursion (goto-char (point-max))
                                    (format-mode-line "%l")))))
    (add-hook 'nlinum-mode-hook #'mikemacs|init-nlinum-width)))
...
(add-hook 'mikemacs-post-init-hook #'winner-mode)
...
(use-package projectile
  :demand t
  :init
  (setq projectile-cache-file (concat mikemacs-cache-dir "projectile.cache")
        projectile-enable-caching t
        projectile-indexing-method 'alien
        projectile-require-project-root nil
        projectile-known-projects-file (concat mikemacs-cache-dir "projectile.projects")
        projectile-globally-ignored-files '(".DS_Store" "Icon" "TAGS")
        projectile-globally-ignored-file-suffixes '(".elc" ".pyc" ".o" ".class"))

  (add-hook 'mikemacs-post-init-hook #'projectile-mode)
  :config
  ...
  (defun mikemacs--projectile-locate-dominating-file (orig-fn &rest args)
    "Don't traverse the file system if on a remote connection."
    (unless (file-remote-p default-directory)
      (apply orig-fn args)))
  (advice-add #'projectile-locate-dominating-file :around #'mikemacs--projectile-locate-dominating-file))

(defun mikemacs-project-root ()
...
(use-package recentf
  :demand t
  :config
  (setq recentf-save-file (concat mikemacs-etc-dir "recentf")
...
(use-package dumb-jump
  ...
  :config
  (setq dumb-jump-default-project mikemacs-emacs-dir
...
(defun mikemacs/jump-definition (&optional other-window)
...
(defun mikemacs/jump-definition-other-window ()
  (interactive)
  (mikemacs/jump-definition t))
#+END_SRC
...
All configurations for Org-mode/Babel/Capture/etc.
...
(defun +borg/org-load-hook (func &optional append)
  (if (featurep 'org)
      (add-hook 'mikemacs-post-init-hook func append)
...
(use-package counsel-dash
  ...
  :config
  (setq counsel-dash-docsets-path (concat mikemacs-etc-dir "docsets")))
#+END_SRC
...
(setq ;; Always copy/delete recursively
      ...
      ;; files
      image-dired-dir (concat mikemacs-cache-dir "image-dired/")
  ...
  (let ((eshell-window (get-buffer-window "*eshell-project*")))
    (if eshell-window
      ...
      (let ((eshell-buffer-name "*eshell-project*")
            (eshell-directory-name (mikemacs-project-root)))
        (eshell)))))
...
(use-package eshell ; built-in
  ...
  :init
  (setq eshell-directory-name (concat mikemacs-etc-dir "/eshell")
        ...
        ;; em-alias
        eshell-aliases-file (concat mikemacs-local-dir ".eshell-aliases"))
...
redefines its keys every time `eshell-mode' is enabled."
    (map! :map eshell-mode-map
          ...
          ;; MIKEMACS - needed to actually send input and use shell
...
(defun +neotree/toggle ()
  ...
  (let ((path buffer-file-name)
        (project-root (mikemacs-project-root)))
    (require 'neotree)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package irony
    ...
    :preface
    (setq irony-server-install-prefix (concat mikemacs-etc-dir "irony-server/"))
    :init
...
(use-package omnisharp
  ...
  (setq omnisharp-auto-complete-want-documentation nil
        omnisharp-server-executable-path (concat mikemacs-local-dir "OmniSharp.exe"))
  :config
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package elisp-mode ; built-in
    ...
    (defun +emacs-lisp|init-flycheck ()
      ...
      (when (and buffer-file-name
                 (not (file-in-directory-p buffer-file-name mikemacs-emacs-dir)))
        (flycheck-mode +1))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package meghanada
    ...
    :config
    (setq meghanada-server-install-dir (concat mikemacs-etc-dir "meghanada-server/")
...
(use-package company-php
  ...
  (setq ac-php-tags-path (concat mikemacs-cache-dir "ac-php/")))
#+END_SRC
...
(use-package plantuml-mode
  :mode "\\.p\\(lant\\)?uml$"
  :config
  (setq plantuml-jar-path (concat mikemacs-etc-dir "plantuml.jar"))
...
(use-package python
  ...
  :init
  (setq python-environment-directory mikemacs-cache-dir
...
(use-package anaconda-mode
  ...
  (setq anaconda-mode-installation-directory (concat mikemacs-etc-dir "anaconda/")
...
#+BEGIN_SRC emacs-lisp
(defvar +rust-src-dir (concat mikemacs-etc-dir "rust/")
...
(use-package tide
  ...
  (defun +typescript|init-tide ()
    (when (or (eq major-mode 'typescript-mode)
      ...
      (flycheck-mode +1)
      (eldoc-mode +1)
      (setq tide-project-root (mikemacs-project-root))))
...
#+BEGIN_SRC emacs-lisp
(cl-defun mikemacs/make-popup (buffer-rx &optional (height 0.4))
    (add-to-list 'mikemacs-popups buffer-rx)
...
#+BEGIN_SRC emacs-lisp
;; Messages
(mikemacs/make-popup (rx bos "*Messages*" eos))
;; Backtrace
(mikemacs/make-popup (rx bos "*Backtrace*" eos))
;; Pretty Print Eval Output
(mikemacs/make-popup (rx bos "*Pp Eval Output*" eos))
;; Help
(mikemacs/make-popup (rx bos "*Help*" eos))
;; Restclient Results
(mikemacs/make-popup (rx bos "*HTTP Response*" eos))
;; Eshell
(mikemacs/make-popup (rx bos "*eshell*" eos))
;; Eshell Project Buffer
(mikemacs/make-popup (rx bos "*eshell-project*" eos))
;; Flycheck Errors
(mikemacs/make-popup (rx bos "*Flycheck errors*" eos))
;; VC Diff
(mikemacs/make-popup (rx bos "*vc-diff*" eos))
;; Org Popups
(mikemacs/make-popup (rx bos "*Calendar*" eos))
(mikemacs/make-popup (rx bos "*Org todo*" eos) 0.05)
(mikemacs/make-popup (rx bos "*Org Note*" eos) 0.1)
(mikemacs/make-popup (rx bos "*Org Select*" eos) 0.2)
(mikemacs/make-popup (rx bos "*Org Links*" eos) 0.05)
(mikemacs/make-popup (rx bos "*Org Export Dispatcher*" eos) 0.05)
(mikemacs/make-popup (rx bos "*Org Commands*" eos) 0.05)
(mikemacs/make-popup (rx bos "*Org Agenda") 0.2)
(mikemacs/make-popup (rx bos "*Agenda Commands*" eos) 0.2)
(mikemacs/make-popup (rx bos "*Org Clock*" eos))
(mikemacs/make-popup (rx bos "*Org Src"))
(mikemacs/make-popup (rx bos "*Edit Formulas*" eos) 0.01)
(mikemacs/make-popup (rx bos "*Org-Babel") 0.25)
(mikemacs/make-popup (rx bos "CAPTURE") 0.2)
;; Scratch Buffer
(mikemacs/make-popup (rx bos "*scratch*" eos) 0.4)
#+END_SRC
...
(defun hydra-move-splitter-down (arg)
 ...
 (defhydra mikemacs-hydra-window ()
   "
...
#+BEGIN_SRC emacs-lisp
    (defhydra mikemacs-hydra-smerge (:hint nil
                            :pre (smerge-mode 1)
...
#+BEGIN_SRC emacs-lisp
  (defhydra mikemacs-hydra-text-zoom (:hint t :color red)
    "
...
#+BEGIN_SRC emacs-lisp
  (defun mikemacs/copy-file ()
    "Write the file under new name."
    (interactive)
    (call-interactively 'write-file))

  (defun mikemacs/rename-file (filename &optional new-filename)
  ...
  (defun mikemacs/rename-current-buffer-file ()
  ...
  (defun mikemacs/delete-file (filename &optional ask-user)
  ...
  (defun mikemacs/delete-file-confirm (filename)
    "Remove specified file or directory after users approval.

  FILENAME is deleted using `mikemacs/delete-file' function.."
    (interactive "f")
    (funcall-interactively #'mikemacs/delete-file filename t))

  (defun mikemacs/delete-current-buffer-file ()
  ...
  (defun mikemacs/sudo-edit (&optional arg)
    (interactive "P")
  ...
  (defun mikemacs/show-and-copy-buffer-filename ()
  ...
  (defun mikemacs/alternate-buffer (&optional window)
  ...
  (defun mikemacs/toggle-fold ()
    (interactive)
    (save-excursion
      (end-of-line)
      (hs-toggle-hiding)))

  (defun mikemacs/sudo-find-file (file)
    "Open a file as root."
    (interactive
  ...
  (defun mikemacs/sudo-this-file ()
    "Open the current file as root."
    (interactive)
    (mikemacs/sudo-find-file (file-truename buffer-file-name)))
#+END_SRC

*** Buffer Functions

#+BEGIN_SRC emacs-lisp
(defun mikemacs/kill-buffer-and-windows (buffer)
...
(defun mikemacs/kill-other-buffers (&optional persp-p)
...
project."
  ...
  (let ((buffers (if persp-p (persp-buffer-list) (buffer-list)))
    ...
    (dolist (buf buffers)
      (unless (eq buf current-buffer)
        (mikemacs/kill-buffer-and-windows buf)))
    (when (called-interactively-p 'interactive)
      (message "Killed %s buffers" (length buffers)))))

 (defun mikemacs//get-buffer-display-time (buffer start-time)
 ...
 (defun mikemacs/show-hide-popup-buffer (&optional select buffer-rx)
   (interactive)
             ...
             (get-buffer-window
               (car
                 (sort
                   open-popup-buffers
                   #'(lambda (a b)
                     (> (mikemacs//get-buffer-display-time a start-time) (mikemacs//get-buffer-display-time b start-time))))))))
         ((> 0 (length open-popup-buffers))
           (ivy-read "Close popup: "
                 (mapcar #'buffer-name open-popup-buffers)
                 :action (lambda (x)
                           (delete-window (get-buffer-window x)))
                 :caller 'mikemacs/show-hide-popup-buffer))
...
(defun mikemacs/toggle-whitespace ()
  (interactive)
  (if (and (boundp 'whitespace-mode) whitespace-mode)
      (whitespace-mode -1)
    (whitespace-mode +1)))

(defun mikemacs/scratch-buffer ()
  (interactive)
...
(defun counsel-projectile-bookmark ()
  ...
  (defun workspace-bookmark-p (bmark)
    (let ((bmark-path (expand-file-name (bookmark-location bmark))))
      (string-prefix-p (mikemacs-project-root) bmark-path)))
#+END_SRC

*** Random Functions

#+BEGIN_SRC emacs-lisp
(defun mikemacs/delete-git-index-lock ()
...
(defun mikemacs/set-limit-auto-fill-column ()
  (interactive)
  (let ((column (string-to-number
                 (read-string "Fill Column: "))))
    (when (> column 0)
...
(defun mikemacs/disable-all-fill-column ()
  (interactive)
  (visual-line-mode -1)
  (auto-fill-mode -1)
  (visual-fill-column-mode -1))
...
(map!
 ...
 ;; Tools
 "C-`"    #'mikemacs/show-hide-popup-buffer
 "C-~"    (λ! (mikemacs/show-hide-popup-buffer t))
 ;; Text-scaling
 ...
 ;; Fold toggle
 :nv [C-backspace] #'mikemacs/toggle-fold
 ...
 :nv [C-S-backspace] #'hs-show-all
  ...
  (:leader
    ;; common
    :desc "Blink cursor line"                :nv "SPC" #'mikemacs/blink-cursor
    ...
    :desc "Switch to last buffer"            :nv "TAB" #'mikemacs/alternate-buffer
    ...
    (:desc "file"
      ...
      :desc "Sudo open file from here"       :nv "F" #'mikemacs/sudo-find-file
      :desc "Copy file"                      :nv "c" #'copy-file
      :desc "Copy current file"              :nv "C" #'mikemacs/copy-file
      :desc "Rename current file"            :nv "R" #'mikemacs/rename-current-buffer-file
      :desc "Delete file"                    :nv "d" #'mikemacs/delete-file-confirm
      :desc "Delete current file"            :nv "D" #'mikemacs/delete-current-buffer-file
      :desc "Sudo edit file"                 :nv "E" #'mikemacs/sudo-edit
      ...
      :desc "Show and copy filename"         :nv "y" #'mikemacs/show-and-copy-buffer-filename
      )
    ...
    (:desc "buffer"
      ...
      :desc "Kill other buffers"             :nv "D" (λ! (mikemacs/kill-other-buffers t))
      :desc "Kill a buffer"                  :nv "k" #'kill-buffer
      :desc "New empty buffer"               :nv "n" #'evil-buffer-new
      :desc "Scratch buffer"                 :nv "s" #'mikemacs/scratch-buffer
    ...
    (:desc "window"
      ...
      :desc "Ace delete window"            :nv "D" #'mikemacs/ace-delete-window-dispatch
      ...
      :desc "Ace window"                   :nv "W" #'ace-window
      :desc "Manage Windows"               :nv "w" #'mikemacs-hydra-window/body
      :desc "Toggle maximize window"       :nv "m" #'mikemacs/window-zoom)

    (:desc "jump"
      :prefix "j"
      :desc "Go definition"                :nv "q" #'mikemacs/jump-definition
      :desc "Go definition other window"   :nv "Q" #'mikemacs/jump-definition-other-window
    ...
    (:desc "git"
      ...
      :desc "Smerge Hydra"                 :nv "m" #'mikemacs-hydra-smerge/body
      :desc "Show and copy git link"       :nv "y" #'git-link
      :desc "Open git link"                :nv "o" #'mikemacs/git-browse
      :desc "Delete git index lock"        :nv "X" #'mikemacs/delete-git-index-lock
      (:desc "gist"
        :prefix "g"
   ...
   (:desc "toggle"
     ...
     :desc "Rainbow mode"                  :nv "r" #'rainbow-mode 
     :desc "Evil goggles"                  :nv "g" #'+evil-goggles/toggle
     :desc "Visual line wrapping"          :nv "v" #'visual-line-mode
     :desc "Whitespace mode"               :nv "w" #'mikemacs/toggle-whitespace)

   (:desc "text"
     :prefix "t"
     :desc "Regular expression builder"    :nv "r" #'regexp-builder
     :desc "Zoom"                          :nv "z" #'mikemacs-hydra-text-zoom/body
     (:desc "fill"
       :prefix "f"
       :desc "Limit fill column"           :nv "l" #'mikemacs/set-limit-fill-column
       :desc "Auto limit fill column"      :nv "a" #'mikemacs/set-limit-fill-column
       :desc "Disable fill column limit"   :nv "d" #'mikemacs/disable-all-fill-column)
     (:desc "html"
       :prefix "h"
...
#+BEGIN_SRC emacs-lisp
(run-hooks 'mikemacs-post-init-hook)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(defun mkh/browse-url (url &optional ignored)
  (interactive (browse-url-interactive-arg "URL: "))
  (call-process "/home/mkh/bin/open-qutebrowser" nil 0 nil url))
(setq browse-url-browser-function #'mkh/browse-url)

(defun mkh/browse-url-new-window (url &optional ignored)
  (call-process "qutebrowser" nil 0 nil "--target" "window" url))
(setq counsel-dash-browser-func #'mkh/browse-url-new-window)
#+END_SRC
